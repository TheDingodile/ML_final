...
class NeuralSQLExecutor:
    def __init__(
        ...
        threshold: float = 0.9,
        model_path_name: str = "params_model_2-0",
    ) -> None:
        ...
        self.vqa_module = get_vqa_module(vqa_module_type, threshold, model_path_name)
    ...
    def _load_sid_to_ipath_map(self) -> Dict[str, str]:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                ipath = self.mimic_cxr_image_dir + "/" + iid + ".jpg"
    ...
    def _execute_select_query(self, query_root: expressions.Select, nsql: str, tables: Dict[str, Table]) -> Union[Table, List[bool]]:
        ...
        if isinstance(from_clause_node, expressions.Select):
            ...
            temp = self._execute_vqa_table_query(query_root, nsql, tables)
            # print(temp)
            return temp
